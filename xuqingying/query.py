...
def Node_id(graph=[]):
        ...
        for node in node1:
            nodes[node]=[]
            nodes[node].extend(Query_Entity_Id(node))
...
def Query_Held_Company(u):
        ...
        #print(res1)
        return res
def Query_Holder(start,end):
                        #''' 查询持股路径\n 输入：持股人，被持股公司\u 输出：持股路径\path[]'''提供控制权所需路径查询'''
...
def Query_Entity_Id(u):
        ...
        sparql= """
                select * where {{
                ?entity <file:///D:/d2rq-0.8.1/vocab/entity_name> "{name}".
                ?entity <file:///D:/d2rq-0.8.1/vocab/entity_id> ?entity_id.
                }}""".format(name=u)
        filename = "res.txt"
...
#data = Query_Held_Company("张宝世")
#print(data)
#data1= Query_Shareholder("长沙泰帮医疗科技有限公司")
#print(data1)

    #print(type(data))"交通银行股份有限公司","平安银行股份有限公司深圳南油支行"#'''#另一组测试实体'''
#data2=Query_Shareholding_Path("张宝世","呼伦贝尔市新村物流运输有限责任公司")
#print(data2)
#print(Query_associate_entity_id("5b98c29f1e6e10b4f9f4fe1a28079160"))
#print(len(data1))