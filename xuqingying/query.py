    ...
    while (i < len(graph)):
        for node in graph[i]:
            if (node not in flag):
                ...
                # print('node',link)
                node1.extend(
                    map(lambda item: (node, item['n']['value'], 'P' if len(item['n']['value']) < 5 else 'E'), link))
                # print(node1)
...
def Query_Entity(u):
    ...
    # print("res2",res2)
    if len(res1) > 0:
        node.extend(list(map(lambda item: {'id': item['associate_entity_id']['value'],
                                           'name': item['associate_entity_name']['value'],
                                           'category': 'P' if len(item['associate_entity_name']['value']) < 5 else 'E'},
                             res1)))

        # print(node)
        link.extend(list(map(lambda item: {'target': item['associate_entity_id']['value'], 'source': entity,
                                           'stake': item['stake']['value']}, res1)))
    if len(res2) > 0:
        node.extend(list(map(lambda item: {'id': item['entity_id']['value'], 'name': item['entity_name']['value'],
                                           'category': 'P' if len(item['entity_name']['value']) < 5 else 'E'}, res2)))
        link.extend(list(
            map(lambda item: {'target': entity, 'source': item['entity_id']['value'], 'stake': item['stake']['value']},
                res2)))
    # print(link)
    return {'nodes': node,
            'links':
...
def Query_Shareholding_Path(start, end):
    ...
    while i < len(paths):
        ...
        stake.extend(
            map(lambda x, y: list(map(lambda item: item['stake']['value'], Query_Held_Stake(x, y))), paths[i], link))
        amount.extend(
            map(lambda x, y: list(map(lambda item: item['amount']['value'], Query_Held_Stake(x, y))), paths[i], link))
        PATH.extend(
            map(lambda x, y, z, w: {'source': x, 'target': y, "stake": z, "amount": w}, paths[i], link, stake, amount))

        def parseItem(item):
            tmp = Query_Held_Stake(item[0], item[1])[0]
            # print('tmp',tmp)
            return {'source': item[0], 'target': item[1], 'stake': tmp['stake']['value'],
                    'amount': tmp['amount']['value']}
...
print(data2)
print(float(data2['links'][1]['stake']))
print(float(data2['links'][1]['stake'][0]))
